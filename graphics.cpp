...
void Graphics::HierarchicalUpdate2(double dt) {
	...
	localTransform *= glm::rotate(ogModel, rotSpeed[0] * (float)dt, rotVector);
	...
	localTransform *= glm::rotate(ogMoon, rotSpeed[1] * (float)dt, rotVector);
	localTransform *= glm::scale(glm::vec3(scale[0], scale[1], scale[2]));
